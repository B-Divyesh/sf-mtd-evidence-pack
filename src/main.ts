...
function landingPage(): string {
  ...
  <section class="pricing" aria-labelledby="licence-title"><div><p class="eyebrow">Supported edition</p><h2 id="licence-title">Keep the core pack free</h2><p>Import records, check the quarter, and export the encrypted pack without paying.</p></div><div class="price-ticket"><h3>Restore an existing licence</h3><p>A verified licence adds saved cover notes and custom checklist items.</p><form data-form="restore-license" class="restore-form"><label for="landing-license">Paste your licence</label><input id="landing-license" name="license" autocomplete="off" required><button class="button small" type="submit" aria-label="Verify licence">Verify licence</button></form></div></section>`, "");
}
...
function workspacePage(): string {
  ...
  const supportNotice = licensed || demoMode ? `<p class="licensed-note">Supported edition features are available${demoMode ? " in this sample" : ""}.</p>` : `<aside class="support-nudge">${licenseNotice ? `<p><strong>${escapeHtml(licenseNotice)}</strong></p>` : ""}<p><strong>Already have a supported-edition licence?</strong></p><p>Verify it to use saved cover notes and custom checklist items.</p><form data-form="restore-license" class="restore-form"><label for="workspace-license">Paste your licence</label><input id="workspace-license" name="license" autocomplete="off" required><button class="button small" type="submit" aria-label="Verify licence">Verify licence</button></form></aside>`;
  ...
  return shell(`<section class="workspace-title"><p class="eyebrow">Local quarterly workspace</p><h1>Prepare this quarter’s evidence pack</h1><p>Your work saves on this device.</p></section>
    ...
    <section class="work-section" aria-labelledby="note-title"><div class="section-lead"><span>05</span><div><h2 id="note-title">Leave a cover note</h2><p>Name any point that needs the accountant’s judgement.</p></div></div><label for="cover-note" class="full-label">Cover note<textarea id="cover-note" data-cover-note rows="4" maxlength="600" ${licensed || demoMode ? "" : "disabled"}>${escapeHtml(workspace.coverNote)}</textarea></label>${!licensed && !demoMode ? `<p class="field-note">Cover notes require a verified supported-edition licence. The free export remains available.</p>` : ""}</section>
...
function privacyPage(): string { return shell(`<article class="legal-page"><p class="eyebrow">Plain-language policy · 28 August 2026</p><h1>Understand your browser data</h1><p>MTD Evidence Pack works in your browser. The demo uses sample data. Review the data controls in your browser before using the workspace with your own records.</p><h2>Manage local data</h2><p>Use “Delete local data” in the workspace to remove its local record. Browser site-data settings can also remove local workspace data. Exported ZIP files are outside the browser and remain yours to manage.</p><h2>Licences</h2><p>Existing supported-edition licences are checked by Sociobot’s licence service.</p><h2>Contact</h2><p>Questions can be sent to <a href="mailto:privacy@sociobot.in">privacy@sociobot.in</a>.</p></article>`); }

function termsPage(): string { return shell(`<article class="legal-page"><p class="eyebrow">Terms · 28 August 2026</p><h1>Use this tool to organise records</h1><p>MTD Evidence Pack helps you prepare a local evidence handoff. Obtain suitable advice and use compatible software or an authorised person where needed.</p><h2>Your responsibility</h2><p>Check dates, categories, records, and checklist items before handoff. Keep original records. Use compatible software or an authorised person for any required submission.</p><h2>Supported edition</h2><p>A refunded, expired, or revoked licence stops supported-edition features. The core export remains available.</p><h2>Availability and liability</h2><p>The tool is provided without a promise that it fits every tax situation. To the extent allowed by law, we are not liable for tax decisions, missed deadlines, or lost local data.</p><h2>Fair use</h2><p>Do not attempt to disrupt the service or use it for unlawful records.</p></article>`); }
