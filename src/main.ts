...
function footer(): string {
  return `<footer class="site-footer"><p>Prepare a quarterly evidence handoff on your device.</p><nav aria-label="Footer navigation"><a href="/privacy" data-link>Privacy</a><a href="/terms" data-link>Terms</a><a href="https://sociobot.in" rel="external">Built by Param Factory <span class="sr-only">(external site)</span></a></nav><p class="build-id">v1.0.0 · Generated art disclosed</p></footer>`;
}
...
function workspacePage(): string {
  ...
  return shell(`<section class="workspace-title"><p class="eyebrow">Local quarterly workspace</p><h1>Prepare this quarter’s evidence pack</h1><p>Your work saves on this device. Nothing is sent to HMRC.</p></section>
    ...
    <section class="work-section" aria-labelledby="check-title"><div class="section-lead"><span>04</span><div><h2 id="check-title">Check the evidence trail</h2><p>Working checklist v1.0 for UK sole traders, 2026–27. Confirm it with your accountant.</p></div></div><ul class="checklist">${checks}</ul><form data-form="custom-check" class="inline-form"><label for="custom-check">Add your own check</label><div><input id="custom-check" name="label" maxlength="100" ${licensed || demoMode ? "" : "disabled"}><button class="button small" type="submit" ${licensed || demoMode ? "" : "disabled"}>Add check</button></div></form>${supportNotice}</section>
    <section class="work-section" aria-labelledby="note-title"><div class="section-lead"><span>05</span><div><h2 id="note-title">Leave a cover note</h2><p>Name any point that needs the accountant’s judgement.</p></div></div><label for="cover-note" class="full-label">Cover note<textarea id="cover-note" data-cover-note rows="4" maxlength="600" ${licensed || demoMode ? "" : "disabled"}>${escapeHtml(workspace.coverNote)}</textarea></label>${!licensed && !demoMode ? `<p class="field-note">Cover notes are in the £24 supported edition. The free export remains available.</p>` : ""}</section>
    <section class="export-section" aria-labelledby="export-title"><div><p class="eyebrow">Final pass</p><h2 id="export-title">Export one encrypted handoff</h2><p>The ZIP contains a CSV, PDF summary, source files, manifest, and SHA-256 file hashes.</p><div class="gaps"><h3>${readiness.gaps.length ? `${readiness.gaps.length} open item${readiness.gaps.length === 1 ? "" : "s"}` : "Ready to hand over"}</h3>${gapList}</div></div><form data-form="export" class="export-form"><label for="pack-password">ZIP password <span>At least 8 characters</span></label><input id="pack-password" name="password" type="password" minlength="8" autocomplete="new-password" required><label for="pack-password-confirm">Repeat password</label><input id="pack-password-confirm" name="confirm" type="password" minlength="8" autocomplete="new-password" required><button class="button primary" type="submit" ${workspace.transactions.length ? "" : "disabled"}>Export encrypted ZIP</button><p>Send the password by a different channel. It is never saved.</p></form></section>
...
function privacyPage(): string { return shell(`<article class="legal-page"><p class="eyebrow">Plain-language policy · 28 August 2026</p><h1>Your records stay in your browser</h1><p>MTD Evidence Pack stores workspace data in this browser’s IndexedDB. It does not send records, files, passwords, or HMRC credentials to us.</p><h2>What is stored</h2><p>Your period, imported records, checklist, cover note, and source files stay on this device. Demo data uses memory only and is discarded when you leave.</p><h2>What leaves the device</h2><p>A licence check sends only the licence token to the Sociobot billing API. Checkout happens on the Sociobot site. The product has no analytics, advertising, or tracking scripts.</p><h2>Your choices</h2><p>Use “Delete local data” in the workspace to erase its local record. Removing site data in browser settings also clears it. Exported ZIP files are outside the browser and remain yours to manage.</p><h2>Contact</h2><p>Questions can be sent to <a href="mailto:privacy@sociobot.in">privacy@sociobot.in</a>.</p></article>`); }

function termsPage(): string { return shell(`<article class="legal-page"><p class="eyebrow">Terms · 28 August 2026</p><h1>Use this tool to organise records</h1><p>MTD Evidence Pack helps you prepare a local evidence handoff. It is not tax advice, filing software, legal certification, or an HMRC service.</p><h2>Your responsibility</h2><p>Check dates, categories, records, and checklist items before handoff. Keep original records. Use compatible software or an authorised person for any required submission.</p><h2>Supported edition</h2><p>The supported edition costs £24 as a one-time purchase. Sociobot/Dodo is the merchant of record. Checkout terms and refunds are handled there. A refunded or revoked licence stops the paid features.</p><h2>Availability and liability</h2><p>The tool is provided without a promise that it fits every tax situation. To the extent allowed by law, we are not liable for tax decisions, missed deadlines, or lost local data.</p><h2>Fair use</h2><p>Do not attempt to disrupt the service or use it for unlawful records.</p></article>`); }
...
app.addEventListener("change", async event => {
  ...
  if (input.matches("[data-import-csv]") && input.files?.[0]) {
    ...
    else { workspace.transactions = result.rows; errorMessage = ""; statusMessage = `${result.rows.length} records imported.`; await persist(); }
    render(); return;
  }
